    ...
    <div class="align-center bg-slate-100 rounded-md w-auto p-4">
        ...
        {{-- <form action="{{ route('barang.update', ['barang' => $barang->id]) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="namaBarang" id="namaBarang" class="border rounded w-full py-2 px-3" value="{{ $barang->namaBarang }}" required>
            </div>

            <div class="mb-4">
                <label for="jenisBarang" class="block text-gray-700 text-sm font-semibold mb-2">Jenis Barang:</label>
                <input type="text" name="jenisBarang" id="jenisBarang" class="border rounded w-full py-2 px-3" value="{{ $barang->jenisBarang }}" required>
            </div>

            <div class="mb-4">
                <label for="jumlahBarang" class="block text-gray-700 text-sm font-semibold mb-2">Jumlah Barang:</label>
                <input type="number" name="jumlahBarang" id="jumlahBarang" class="border rounded w-full py-2 px-3" value="{{ $barang->jumlahBarang }}" required>
            </div>

            <div class="mb-4">
                <label for="tanggalMasukBarang" class="block text-gray-700 text-sm font-semibold mb-2">Tanggal Masuk Barang:</label>
                <input type="date" name="tanggalMasukBarang" id="tanggalMasukBarang" class="border rounded w-full py-2 px-3" value="{{ $barang->tanggalMasukBarang }}" required>
            </div>
